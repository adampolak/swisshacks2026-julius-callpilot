...
The live page no longer replays `Gi/conversation_transcript.md`. `Start call`
starts the server-owned Deepgram duplex transcriber and the browser polls the
finalized/interim turns from `/api/transcription`.

Install the audio dependencies in the Python environment used for `server.py`:

```powershell
pip install -r requirements-transcription.txt
```

Set the Deepgram key and, if needed, override the aggregate-device mapping:

```powershell
$env:DG_API_KEY = "your_deepgram_key"
$env:TRANSCRIBE_DEVICE = "Aggregate In"
$env:TRANSCRIBE_TOTAL_CH = "3"
$env:TRANSCRIBE_MIC_CH = "0"
$env:TRANSCRIBE_SYS_CH = "1,2"
```

The defaults match `transcribe-api/deepgram_double_source.py`: channel 0 is the
RM microphone and channels 1-2 are system audio from the client.
...
The live transcript renders finalized Deepgram turns with explicit RM and Client
attribution. Steering appears only when a complete turn contains a meaningful
signal.
...
The prototype still reads `Gi/customer_profile.md` and `Gi/system_prompt.md` at
startup for CLM context and steering rules. `Gi/conversation_transcript.md` is no
longer used for the live call feed. `Gi/evaluation_key.md` is reserved for
